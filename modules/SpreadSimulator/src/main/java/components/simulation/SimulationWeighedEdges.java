...
public class SimulationWeighedEdges extends Simulation {

    public SimulationWeighedEdges(Graph graph, SimulationModel simulationModel) {
        super(graph, simulationModel);
    }

    @Override
    protected boolean Validate(){
        if(!graph.getModel().getEdgeTable().hasColumn("WEIGHT")){
            JOptionPane.showMessageDialog(null, "Add WEIGHT column statistic to all edges in graph first. You can use RandomEdgeWeight statistic to add this automatically.", "Error", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        return true;
    }
    ...
    private void ConditionProbabilityNode(Graph graph, Node node, Transition transition) {
        ...
        var selectedEdges = edgesOfNode.toCollection().stream().filter(edge -> {
            ...
            var wcnAttribute = edge.getAttribute("WEIGHT");
